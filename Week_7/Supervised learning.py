...
train_X, val_X, train_y, val_y = train_test_split(
    X, y, random_state=0, train_size=0.8) #not fixed, random state shuffles the data, best random state to use is state 42, it gives the best shuffle
...
def evaluate_model(model, val_X14, val_y14):
    ...
    print(f'Model accuracy: {accuracy}')
    y14_pred_proba = model.predict_proba(val_X14)[:, 1]
